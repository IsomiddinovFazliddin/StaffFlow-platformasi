...
export default function SignUp() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (result.pending) { navigate('/pending', { replace: true }); return; }
    // Birinchi foydalanuvchi → admin dashboard
    if (result.user) {
  ...
  const handleGoogle = async () => {
    ...
    if (!result) return; // popup yopildi
    ...
    // Birinchi foydalanuvchi = admin, darhol kiradi
    const role = result.user?.role;
    navigate(role === 'employee' ? '/employee' : '/admin', { replace: true });
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-white to-slate-100 dark:from-slate-900 dark:via-slate-900 dark:to-slate-800 flex items-center justify-center p-4">
      <div className="w-full max-w-sm">
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl dark:shadow-slate-900/50 border border-slate-100 dark:border-slate-700 p-8">
          ...
          {/* Info banner */}
          <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 text-blue-700 dark:text-blue-400 text-xs px-3.5 py-2.5 rounded-xl mb-5">
            ℹ️ Ro'yxatdan o'tgandan so'ng Admin tasdiqlashini kuting.
          </div>
          ...
          >
            ...
            {gLoading ? 'Yuklanmoqda...' : 'Google orqali ro\'yxatdan o\'tish'}
          </button>
