  ...
  try {
    ...
    const { rows } = await query(
      `SELECT u.id, u.full_name, u.email, u.role, u.position, u.salary,
       ...
       WHERE u.id IN ${clause} AND u.status = 'active' AND u.role != 'admin'
...
router.post('/', protect, allow('admin'), async (req, res) => {
  try {
    ...
    const { rows } = await query(
      ...
      [full_name, email.toLowerCase(), hash, role || 'employee',
       department_id || null, salary || 0, position || '', phone || '']
    );
    res.status(201).json({ employee: rows[0] || { full_name, email } });
  } catch (err) {
...
router.put('/:id', protect, allow('admin'), async (req, res) => {
  try {
    ...
    const { rows } = await query(
      `SELECT id, full_name, email, role, department_id, salary, position, phone FROM users WHERE id = $1`,
      [req.params.id]
    );
